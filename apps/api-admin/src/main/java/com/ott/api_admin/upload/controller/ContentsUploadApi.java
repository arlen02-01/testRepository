...
@Tag(name = "Contents Upload API", description = "[백오피스] 콘텐츠 업로드 준비 API")
public interface ContentsUploadApi {

    @Operation(
            summary = "콘텐츠 업로드 요청",
            description = "콘텐츠 메타데이터를 저장하고 poster/thumbnail/origin/transcoded 업로드용 presigned URL 을 발급합니다."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "콘텐츠 생성 및 presigned URL 발급 성공",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ContentsUploadInitResponse.class)
                    )
            ),
            @ApiResponse(
                    responseCode = "400",
